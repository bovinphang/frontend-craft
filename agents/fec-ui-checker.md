...
1. 先定位最小可复现路径
2. 若项目存在 `design-system/<project>/pages/<page>.md`，先读取页面 override；再读取对应 `design-system/<project>/MASTER.md`
3. 检查组件结构和样式来源
4. 追踪问题来自 markup、CSS、Token、状态还是数据
5. 给出最小且稳妥的修复方案
6. 修改后进行验证
...
1. 从可用的设计工具获取设计数据（颜色、字体、间距、尺寸）
   ...
   - 摹客：从用户提供的截图或标注中获取
2. 读取对应的实现代码
3. 如果存在设计系统文档，确认实现是否遵守 Master 与页面 override 的产品类型、token、图表、动效和 QA 规则
4. 逐项对比以下维度:
...
- 是否忽略 `design-system/MASTER.md` 或页面 override 中已定义的行业风险、图表选择、状态覆盖和交付前 QA
