...
1. 使用本仓库打出的 **`frontend-craft-openclaw`** npm 包：执行 `npm run pack:openclaw` 后，在**仓库根目录**取得 `frontend-craft-openclaw-<version>.tgz` 再安装（例如 `npm install ./frontend-craft-openclaw-2.1.1.tgz`）。
