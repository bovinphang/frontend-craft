...
1. 使用本仓库打出的 **`frontend-craft`** npm 包：执行 `npm run pack:openclaw` 后，在 **`npm-packages/openclaw/`** 目录取得 `frontend-craft-<version>.tgz` 再安装（例如 `npm install ./frontend-craft-2.1.1.tgz`）。
