...
- **`fec-image-generation`**: 技术图质量：完整 Unicode 换行、内容驱动布局、避障连线与可见自调用；修复独立 SVG 样式、真实浏览器 PNG 测量和缩放 manifest，增强质量检查并加入可选本地 Mermaid 入口与三类图例。
- **文档**：新增中英文审核范围与技术图交付指南，同步运行时入口及多语言 README 能力章节，说明工具前提与 QA 边界。
