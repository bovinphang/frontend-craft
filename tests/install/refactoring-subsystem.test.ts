...
function assertRelativeMarkdownLinksResolve(relativePath: string): void {
  ...
  for (const match of body.matchAll(/\]\(([^)#]+\.md)(?:#[^)]+)?\)/g)) {
    assert.ok(
      fs.existsSync(path.resolve(base, match[1])),
...
test("code smell catalog contains exactly 24 canonical smells", () => {
  const rootCatalog = read(
    "skills/fec-code-smells/references/smell-catalog.md",
  );
  const zhCatalog = read(
    "localized/zh-CN/skills/fec-code-smells/references/smell-catalog.md",
  );

  assert.deepEqual(
    extractIds(rootCatalog, "SMELL-").sort(),
    [...canonicalSmellIds].sort(),
  );
  assert.deepEqual(
    extractIds(zhCatalog, "SMELL-").sort(),
    [...canonicalSmellIds].sort(),
  );

  const map = read(
    "skills/fec-code-smells/references/smell-to-refactoring-map.md",
  );
  ...
  const detection = read(
    "skills/fec-code-smells/references/detection-guide.md",
  );
  assert.match(
    detection,
...
const canonicalRefactoringIds = [
  ...
  "RF-MOVE-FUNCTION",
  "RF-MOVE-FIELD",
  "RF-MOVE-STATEMENTS-INTO-FUNCTION",
  "RF-MOVE-STATEMENTS-TO-CALLERS",
  "RF-REPLACE-INLINE-CODE-WITH-FUNCTION-CALL",
  "RF-SLIDE-STATEMENTS",
  "RF-SPLIT-LOOP",
  ...
  "RF-PARAMETERIZE-FUNCTION",
  "RF-REMOVE-FLAG-ARGUMENT",
  "RF-PRESERVE-WHOLE-OBJECT",
  "RF-REPLACE-PARAMETER-WITH-QUERY",
  "RF-REPLACE-QUERY-WITH-PARAMETER",
  "RF-REMOVE-SETTING-METHOD",
  "RF-REPLACE-CONSTRUCTOR-WITH-FACTORY-FUNCTION",
  "RF-REPLACE-FUNCTION-WITH-COMMAND",
  "RF-REPLACE-COMMAND-WITH-FUNCTION",
  "RF-PULL-UP-METHOD",
  "RF-PULL-UP-FIELD",
  "RF-PULL-UP-CONSTRUCTOR-BODY",
  "RF-PUSH-DOWN-METHOD",
  "RF-PUSH-DOWN-FIELD",
...
test("refactoring catalog indexes exactly 61 canonical techniques", () => {
  const rootIndex = read(
    "skills/fec-refactoring-catalog/references/catalog-index.md",
  );
  const zhIndex = read(
    "localized/zh-CN/skills/fec-refactoring-catalog/references/catalog-index.md",
  );
  assert.deepEqual(
    extractIds(rootIndex, "RF-").sort(),
    [...canonicalRefactoringIds].sort(),
  );
  assert.deepEqual(
    extractIds(zhIndex, "RF-").sort(),
    [...canonicalRefactoringIds].sort(),
  );
  assert.equal(extractIds(rootIndex, "RF-").length, 61);
  assert.equal(extractIds(zhIndex, "RF-").length, 61);
  for (const localeRoot of ["skills", "localized/zh-CN/skills"]) {
    const indexPath = `${localeRoot}/fec-refactoring-catalog/references/catalog-index.md`;
    const index = read(indexPath);
    const detailLinks = [
      ...index.matchAll(/\]\((\.\.\/\.\.\/fec-refactoring-[^)]+\.md)\)/g),
    ];
    assert.equal(
      detailLinks.length,
      61,
      `${indexPath}: every technique needs a detail link`,
    );
    assertRelativeMarkdownLinksResolve(indexPath);
  }

  const selection = read(
    "skills/fec-refactoring-catalog/references/selection-guide.md",
  );
  for (const smell of [
    "Long Function",
    "Long Parameter List",
  ...
  const composition = read(
    "skills/fec-refactoring-catalog/references/composition-guide.md",
  );
  assert.match(composition, /RF-EXTRACT-FUNCTION[\s\S]*RF-MOVE-FUNCTION/);
  assert.match(
    composition,
    /RF-REPLACE-TEMP-WITH-QUERY[\s\S]*RF-EXTRACT-FUNCTION/,
  );
});
...
test("function refactoring reference covers 11 canonical techniques", () => {
  ...
  const zh = read(
    "localized/zh-CN/skills/fec-refactoring-functions/references/function-refactorings.md",
  );
  assert.deepEqual(
    extractIds(body, "RF-").sort(),
    [...functionRefactoringIds].sort(),
  );
  assert.deepEqual(
    extractIds(zh, "RF-").sort(),
    [...functionRefactoringIds].sort(),
  );
  ...
  assert.match(
    body,
    /Combine Functions into Class[\s\S]*(hook|composable|service|module)/i,
  );
  assert.match(body, /Split Phase[\s\S]*(parse|normalize|validate|submit)/i);
  assertRelativeMarkdownLinksResolve(
    "skills/fec-refactoring-functions/SKILL.md",
  );
});
...
test("encapsulation reference covers 9 canonical techniques", () => {
  ...
  const zh = read(
    "localized/zh-CN/skills/fec-refactoring-encapsulation/references/encapsulation-refactorings.md",
  );
  assert.deepEqual(
    extractIds(body, "RF-").sort(),
    [...encapsulationRefactoringIds].sort(),
  );
  assert.deepEqual(
    extractIds(zh, "RF-").sort(),
    [...encapsulationRefactoringIds].sort(),
...
test("moving reference covers 9 canonical techniques and cleanup bridge", () => {
  ...
  const zh = read(
    "localized/zh-CN/skills/fec-refactoring-move-features/references/moving-refactorings.md",
  );
  assert.deepEqual(
    extractIds(body, "RF-").sort(),
    [...movingRefactoringIds].sort(),
  );
  assert.deepEqual(
    extractIds(zh, "RF-").sort(),
    [...movingRefactoringIds].sort(),
...
test("data reference covers 5 canonical techniques", () => {
  ...
  const zh = read(
    "localized/zh-CN/skills/fec-refactoring-data/references/data-refactorings.md",
  );
  assert.deepEqual(
    extractIds(body, "RF-").sort(),
    [...dataRefactoringIds].sort(),
  );
  assert.deepEqual(
    extractIds(zh, "RF-").sort(),
    [...dataRefactoringIds].sort(),
  ...
  assert.match(
    body,
    /Replace Derived Variable with Query[\s\S]*(memoization|recomput)/i,
  );
  assert.match(
    body,
    /Change Reference to Value[\s\S]*(React|Vue|identity|reactiv)/i,
  );
});
...
test("conditional reference covers 6 canonical techniques", () => {
  ...
  const zh = read(
    "localized/zh-CN/skills/fec-refactoring-control/references/conditional-refactorings.md",
  );
  assert.deepEqual(
    extractIds(body, "RF-").sort(),
    [...controlRefactoringIds].sort(),
  );
  assert.deepEqual(
    extractIds(zh, "RF-").sort(),
    [...controlRefactoringIds].sort(),
  ...
  assert.match(
    body,
    /Replace Conditional with Polymorphism[\s\S]*(discriminated union|strategy map|component registry|handler)/i,
  );
  assert.match(
    body,
    /Introduce Assertion[\s\S]*(not a substitute|does not replace|user-input validation)/i,
  );
...
test("api reference covers 10 canonical techniques", () => {
  ...
  const zh = read(
    "localized/zh-CN/skills/fec-refactoring-api/references/api-refactorings.md",
  );
  assert.deepEqual(
    extractIds(body, "RF-").sort(),
    [...apiRefactoringIds].sort(),
  );
  assert.deepEqual(extractIds(zh, "RF-").sort(), [...apiRefactoringIds].sort());
  assert.match(
    body,
    /Remove Flag Argument[\s\S]*(behavior|control instruction|ordinary domain)/i,
  );
  assert.match(
    body,
    /Replace Parameter with Query[\s\S]*(hidden dependency|testability|purity)/i,
  );
  assert.match(
    body,
    /Replace Query with Parameter[\s\S]*(hidden dependency|testability|purity)/i,
  );
  assert.match(
    body,
    /Replace Function with Command[\s\S]*(undo|queue|retry|audit|lifecycle)/i,
  );
...
test("inheritance reference covers 11 canonical techniques", () => {
  ...
  const zh = read(
    "localized/zh-CN/skills/fec-refactoring-inheritance/references/inheritance-refactorings.md",
  );
  assert.deepEqual(
    extractIds(body, "RF-").sort(),
    [...inheritanceRefactoringIds].sort(),
  );
  assert.deepEqual(
    extractIds(zh, "RF-").sort(),
    [...inheritanceRefactoringIds].sort(),
  ...
  assert.match(
    body,
    /Replace Type Code with Subclasses[\s\S]*(discriminated union|strategy)/i,
  );
});
...
test("refactoring skills are registered in metadata evals and relations", () => {
  const metadata = JSON.parse(read("skills/metadata.json")) as Array<{
    id: string;
    category: string;
  }>;
  const evals = JSON.parse(read("skills/eval_queries.json")) as Record<
    string,
    { should_trigger: string[]; should_not_trigger: string[] }
  >;
  const relations = JSON.parse(read("skills/relations.json")) as Record<
    string,
    unknown
  >;
  const zhMetadata = JSON.parse(
    read("localized/zh-CN/skills/metadata.json"),
  ) as Array<{ id: string; category: string }>;
  const zhEvals = JSON.parse(
    read("localized/zh-CN/skills/eval_queries.json"),
  ) as Record<
    string,
    { should_trigger: string[]; should_not_trigger: string[] }
  >;
  const zhRelations = JSON.parse(
    read("localized/zh-CN/skills/relations.json"),
  ...
  assert.match(
    read("skills/fec-tdd-workflow/SKILL.md"),
    /Pure behavior-preserving refactoring does not manufacture a failing test/,
  );
  assert.match(
    read("localized/zh-CN/skills/fec-tdd-workflow/SKILL.md"),
    /纯粹的保持行为重构，不应人为制造一个失败测试/,
  );
  assert.match(
    read("commands/fec-tdd.md"),
    /pure behavior-preserving refactoring/i,
  );
  assert.match(
    read("skills/fec-refactor-clean/SKILL.md"),
    /structural refactoring/i,
  );
  assert.match(
    read("agents/fec-code-reviewer.md"),
    /fec-smell|fec-refactor-plan/,
  );
});
...
test("adjacent workflows preserve refactoring routing boundaries", () => {
  ...
  assert.match(
    read("localized/zh-CN/commands/fec-tdd.md"),
    /纯.*保持行为.*重构/,
  );
  assert.match(
    read("skills/fec-refactor-clean/SKILL.md"),
  ...
  assert.match(
    read("localized/zh-CN/skills/fec-refactor-clean/SKILL.md"),
    /结构.*重构/,
  );
  assert.match(
    read("agents/fec-refactor-cleaner.md"),
    /structural refactoring/i,
  );
  assert.match(
    read("localized/zh-CN/agents/fec-refactor-cleaner.md"),
    /结构.*重构/,
  );
  assert.match(
    read("skills/fec-code-review/SKILL.md"),
    /smell diagnosis|refactoring plan/i,
  );
  assert.match(
    read("localized/zh-CN/skills/fec-code-review/SKILL.md"),
    /坏味道诊断|重构计划/,
  );
  assert.match(
    read("agents/fec-code-reviewer.md"),
  ...
  assert.match(
    read("localized/zh-CN/agents/fec-code-reviewer.md"),
    /fec-smell|fec-refactor-plan/,
  );
});
...
test("public docs expose the refactoring subsystem consistently", () => {
  for (const file of [
    ...
    assert.match(
      body,
      /14 specialized agents|14 个.*Agent|14 个.*agent|14 个.*智能体/i,
    );
...
test("refactoring release metadata is synchronized at 2.9.0", () => {
  ...
  for (const file of [
    ...
    const parsed = JSON.parse(read(file)) as {
      version?: string;
      plugins?: Array<{ version?: string; source?: { version?: string } }>;
    };
  ...
  for (const file of [
    ...
    "localized/zh-CN/skills/metadata.json",
  ]) {
    const metadata = JSON.parse(read(file)) as Array<{ version: string }>;
    assert.ok(
      metadata.every((entry) => entry.version === "2.9.0"),
      `${file} versions must be 2.9.0`,
    );
  }
});
...
test("all seven family references define exactly the 61 catalog techniques", () => {
  ...
  const defined = familyFiles.flatMap((file) =>
    [...read(file).matchAll(/^## (RF-[A-Z0-9-]+) — /gm)].map(
      (match) => match[1],
    ),
  );
  ...
  for (const file of familyFiles)
    assertRelativeMarkdownLinksResolve(
      file.replace(/\/references\/[^/]+$/, "/SKILL.md"),
    );
});
...
test("every smell has candidates and dead code keeps cleanup execution ownership", () => {
  ...
  const sections = mapping
    .split(/^## /m)
    .filter((section) => section.startsWith("SMELL-"));
  ...
  for (const section of sections) {
    ...
    assert.match(
      section,
      /RF-[A-Z0-9-]+/,
      `${smellId} must include at least one candidate`,
    );
  }
  assert.match(
    read(
...
test("refactoring agent skills and public counts are internally complete", () => {
  ...
  const skillIds = [...frontmatter.matchAll(/^ {2}- (fec-[a-z0-9-]+)$/gm)].map(
    (match) => match[1],
  );
  for (const skillId of skillIds) assertExists(`skills/${skillId}/SKILL.md`);

  const rootSkillDirs = fs
    .readdirSync(path.join(root, "skills"), { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name);
  const zhSkillDirs = fs
    .readdirSync(path.join(root, "localized/zh-CN/skills"), {
      withFileTypes: true,
    })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name);
