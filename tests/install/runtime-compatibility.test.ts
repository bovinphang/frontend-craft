...
import { getInstallBaseDir, getGlobalConfigDir } from "../../src/install/runtime-homes.js";
...
import { beginManifestSession, endManifestSession, readInstallManifest, writeUtf8 } from "../../src/install/shared/fs.js";
import { uninstallManagedInstall, copyInstallToNewBase } from "../../src/install/manifest-installs.js";
...
function fixture(runtime: string, isGlobal = false) {
  ...
  const baseDir = isGlobal ? path.join(dir, "user-config") : getInstallBaseDir({ runtime, isGlobal, cwd });
  ...
  put(path.join(root, "skills/fec-example/SKILL.md"), "---\nname: fec-example\ndescription: Example skill\n---\nExample\n");
  put(path.join(root, "commands/fec-init.md"), "---\ndescription: Initialize a project\nargument-hint: project\n---\nInitialize $ARGUMENTS\n");
  put(path.join(root, "agents/fec-review.md"), "---\nname: fec-review\ndescription: Review\n---\nReview code\n");
  put(path.join(root, "templates/shared/rules/fec-testing.md"), "Run the project tests.\n");
  put(path.join(root, "templates/claude/CLAUDE.md"), "# Project\n@./rules/fec-testing.md\n");
  put(path.join(root, "templates/claude/settings.json"), '{"permissions":{"allow":[]}}');
  put(path.join(root, "templates/codex/config.toml"), "# Template\n");
  put(path.join(root, "templates/openclaw/AGENTS.md"), "# Workspace\n");
  put(path.join(root, "hooks/hooks.json"), JSON.stringify({ hooks: { Stop: [{ hooks: [{ type: "command", command: 'node "${CLAUDE_PLUGIN_ROOT}/dist/hooks/fec-run-tests.js"' }] }] } }));
  for (const name of ["fec-security-check.js", "fec-format-changed-file.js", "fec-run-tests.js", "fec-notify.js"]) {
    put(path.join(root, "dist/hooks", name), "// hook\n");
  }
  const ctx: InstallContext = { pluginRoot: root, contentRoot: root, cwd, baseDir, runtime, isGlobal, dryRun: false, mode: "install", language: "en" };
  return { root, cwd, baseDir, ctx, put, read: (file: string) => fs.readFileSync(file, "utf8"), cleanup: () => fs.rmSync(dir, { recursive: true, force: true }) };
}
...
for (const runtime of ["claude", "qoder"]) {
  test(`${runtime} merges hooks into settings, preserving unrelated settings and avoiding duplicates`, async () => {
    ...
    try {
      ...
      const custom = { matcher: "custom", hooks: [{ type: "command", command: "echo custom" }] };
      f.put(dest, JSON.stringify({ permissions: { allow: ["custom"] }, hooks: { Stop: [custom] } }));
...
test("Qoder upgrades duplicate legacy global hook paths to one resolvable hook", async () => {
  ...
  try {
    const dest = path.join(f.baseDir, "settings.json");
    const legacy = { matcher: ".*", hooks: [{ type: "command", command: 'node ".qoder/hooks/fec-run-tests.js"', statusMessage: "Running final validation..." }] };
...
for (const isGlobal of [false, true]) {
  test(`Gemini discovers skills and context imports (${isGlobal ? "global" : "local"})`, async () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(f.baseDir, "skills/fec-example/SKILL.md")));
      const context = path.join(isGlobal ? f.baseDir : f.cwd, "GEMINI.md");
      const imports = [...f.read(context).matchAll(/^@(.+)$/gm)];
      assert.ok(imports.length > 0);
      for (const entry of imports) assert.ok(fs.existsSync(path.resolve(path.dirname(context), entry[1].trim())));
    } finally { f.cleanup(); }
  });
  test(`Windsurf workflows use the discovery path (${isGlobal ? "global" : "local"})`, async () => {
    ...
    try {
      ...
      assert.ok(fs.existsSync(path.join(f.baseDir, isGlobal ? "global_workflows" : "workflows", "fec-init.md")));
    } finally { f.cleanup(); }
  });
  test(`Trae installs explicitly activated rules (${isGlobal ? "global" : "local"})`, async () => {
    ...
    try {
      ...
      const rules = f.read(path.join(f.baseDir, isGlobal ? "user_rules" : "rules", "frontend-craft.md"));
      assert.match(rules, /^---\nalwaysApply: true\n---/);
    } finally { f.cleanup(); }
  });
  test(`OpenClaw skills and commands have SKILL.md (${isGlobal ? "global" : "local"})`, async () => {
    ...
    try {
      ...
      assert.match(f.read(path.join(skills, "fec-init/SKILL.md")), /^---\nname: fec-init\n/);
...
test("Copilot project prompts have the required suffix and rules have applyTo", async () => {
  ...
  try {
    ...
    assert.match(f.read(path.join(f.baseDir, "prompts/fec-init.prompt.md")), /Initialize/);
    assert.match(f.read(path.join(f.baseDir, "instructions/frontend-craft.instructions.md")), /^---\napplyTo: "\*\*"\n---/);
  } finally { f.cleanup(); }
});
...
test("Copilot global context is installed without claiming IDE global prompts", async () => {
  ...
  try {
    await INSTALLERS.copilot(f.ctx);
    assert.match(f.read(path.join(f.baseDir, "copilot-instructions.md")), /Run the project tests/);
...
test("OpenCode upgrades the exact legacy template without changing user config", async () => {
  ...
  try {
    ...
    f.put(dest, JSON.stringify({ $schema: "https://opencode.ai/config.json", permissions: { bash: "allow" } }));
    await INSTALLERS.opencode(f.ctx);
...
test("OpenCode update retires unchanged legacy commands but preserves user edits", async () => {
  ...
  try {
    ...
    assert.ok(readInstallManifest(path.join(f.baseDir, "frontend-craft.manifest.json"))?.files.some(file => file.path === "command/custom.md"));
  } finally { f.cleanup(); }
});
...
for (const runtime of ["claude", "qoder"]) {
  test(`${runtime} uninstall removes its hook entries but retains user settings`, async () => {
    ...
    try {
      const dest = path.join(f.baseDir, "settings.json");
      const before = { permissions: { allow: ["custom"] }, hooks: { Stop: [{ hooks: [{ type: "command", command: "echo custom" }] }] } };
      ...
      uninstallManagedInstall({ install: { runtime, scope: "local", baseDir: f.baseDir, manifestPath, manifest }, cwd: f.cwd, dryRun: false, force: false });
...
test("Cline coexists with an existing modern rules directory", async () => {
  ...
  try {
    ...
    assert.equal(f.read(path.join(f.cwd, ".clinerules/custom.md")), "User rule\n");
    assert.match(f.read(path.join(f.cwd, ".clinerules/frontend-craft.md")), /Run the project tests/);
  } finally { f.cleanup(); }
});
...
test("Cline migrates a legacy rules file without losing user content", async () => {
  ...
  try {
    ...
    await INSTALLERS.cline(f.ctx);
    assert.equal(f.read(path.join(f.cwd, ".clinerules/legacy.md")), "User rule\n");
    assert.equal(f.read(path.join(f.cwd, ".clinerules.frontend-craft-backup")), "User rule\n");
    assert.match(f.read(path.join(f.cwd, ".clinerules/frontend-craft.md")), /Run the project tests/);
  } finally { f.cleanup(); }
});
...
test("Cline global rules are installed into the configured rules directory", async () => {
  ...
  try {
    await INSTALLERS.cline(f.ctx);
    assert.match(f.read(path.join(f.baseDir, "Rules/frontend-craft.md")), /Run the project tests/);
  } finally { f.cleanup(); }
});
...
test("OpenClaw workspace initialization emits a discoverable rules skill", () => {
  ...
  try {
    ...
    assert.ok(result.ok);
    assert.match(f.read(path.join(f.cwd, "skills/frontend-craft-rules/SKILL.md")), /^---\nname: frontend-craft-rules\n/);
  } finally { f.cleanup(); }
});
...
test("Antigravity defaults to the current IDE global skills base", () => {
  ...
  try { assert.equal(getGlobalConfigDir("antigravity"), path.join(os.homedir(), ".gemini/config")); }
  finally { if (previous !== undefined) process.env.ANTIGRAVITY_CONFIG_DIR = previous; }
});
...
test("global directory migration carries edited skills with their original ownership hashes", () => {
  ...
  try {
    ...
    beginManifestSession({ ...f.ctx, baseDir: source, packageVersion: "2.9.0" });
    ...
    const manifest = readInstallManifest(manifestPath)!;
    copyInstallToNewBase({ runtime: "antigravity", scope: "global", baseDir: source, manifestPath, manifest }, target, false);
    assert.equal(f.read(path.join(target, file)), "user edit");
    assert.deepEqual(readInstallManifest(path.join(target, "frontend-craft.manifest.json"))?.files, manifest.files);
...
test("OpenClaw respects its official state directory environment variable", () => {
  ...
  try { assert.equal(getGlobalConfigDir("openclaw"), process.env.OPENCLAW_STATE_DIR); }
  finally { if (previous === undefined) delete process.env.OPENCLAW_STATE_DIR; else process.env.OPENCLAW_STATE_DIR = previous; }
});
...
test("OpenClaw preserves full same-name skills and quotes command descriptions", async () => {
  const f = fixture("openclaw");
  try {
    f.put(path.join(f.root, "commands/fec-example.md"), "---\ndescription: Short command\n---\nShort\n");
    f.put(path.join(f.root, "commands/fec-colon.md"), "---\ndescription: Debug: inspect errors\n---\nDebug\n");
    await INSTALLERS.openclaw(f.ctx);
    assert.match(f.read(path.join(f.cwd, "skills/fec-example/SKILL.md")), /Example skill/);
    assert.match(f.read(path.join(f.cwd, "skills/fec-colon/SKILL.md")), /description: "Debug: inspect errors"/);
  } finally { f.cleanup(); }
});
...
for (const runtime of ["gemini", "openclaw"]) {
  test(`${runtime} retires only unchanged owned legacy skills`, async () => {
    const f = fixture(runtime);
    try {
      const oldRoot = runtime === "gemini" ? path.join(f.baseDir, "extensions/frontend-craft/skills") : path.join(f.baseDir, "skills");
      ...
      assert.ok(fs.existsSync(path.join(newRoot, "skills/fec-example/SKILL.md")));
    } finally { f.cleanup(); }
  });
}
