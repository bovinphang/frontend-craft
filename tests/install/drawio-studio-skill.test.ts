...
test("diagram-lint accepts valid drawio and fails dangling edges", () => {
  withTempDir((dir) => {
    ...
    const goodResult = JSON.parse(
      run("diagram-lint.mjs", [good, "--format", "json"]),
    ) as { ok: boolean };
    assert.equal(goodResult.ok, true);
    const failed = spawnSync(
      process.execPath,
      [path.join(scriptsDir, "diagram-lint.mjs"), bad, "--format", "json"],
      {
        encoding: "utf8",
      },
    );
...
test("diagram-url creates drawio import URLs for xml, mermaid, and csv", () => {
  withTempDir((dir) => {
    ...
    const xml = JSON.parse(
      run("diagram-url.mjs", [xmlFile, "--create", "--type", "xml", "--json"]),
    ) as {
      url: string;
      mode: string;
      type: string;
...
test("diagram-url rejects non-xml inputs without create mode", () => {
  withTempDir((dir) => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(scriptsDir, "diagram-url.mjs"), file, "--type", "mermaid"],
      {
        encoding: "utf8",
      },
    );
...
test("diagram-lint reports xml comments, missing roots, unescaped entities, and html label warnings", () => {
  withTempDir((dir) => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(scriptsDir, "diagram-lint.mjs"), file, "--format", "json"],
      {
        encoding: "utf8",
      },
    );
    ...
    assert.ok(
      report.errors.some((message) => /comments are not allowed/.test(message)),
    );
    assert.ok(
      report.errors.some((message) => /missing root cell id 0/.test(message)),
    );
    assert.ok(
      report.errors.some((message) =>
        /missing default layer cell id 1/.test(message),
      ),
    );
    assert.ok(
      report.errors.some((message) => /unescaped ampersand/.test(message)),
    );
    assert.ok(
      report.errors.some(
        (message) =>
          /self-closing/.test(message) || /missing mxGeometry/.test(message),
      ),
    );
...
test("diagram-lint reports layout and label quality warnings", () => {
  withTempDir((dir) => {
    ...
    const report = JSON.parse(
      run("diagram-lint.mjs", [file, "--format", "json"]),
    ) as {
      ok: boolean;
      errors: string[];
      warnings: string[];
    ...
    assert.ok(
      report.warnings.some((message) => /whiteSpace=wrap/.test(message)),
    );
    assert.ok(report.warnings.some((message) => /html=1/.test(message)));
...
test("diagram-lint reports flowchart readability warnings", () => {
  withTempDir((dir) => {
    ...
    assert.ok(
      report.warnings.some((message) => /title fontSize 13/.test(message)),
    );
    assert.ok(report.warnings.some((message) => /align=center/.test(message)));
    assert.ok(
      report.warnings.some((message) => /orthogonalEdgeStyle/.test(message)),
    );
  });
});
...
test("diagram-lint strict mode fails on overlap warnings", () => {
  withTempDir((dir) => {
    ...
    const result = spawnSync(
      process.execPath,
      [
        path.join(scriptsDir, "diagram-lint.mjs"),
        file,
        "--format",
        "json",
        "--strict",
...
test("drawio-export dry-run returns command with export options", () => {
  withTempDir((dir) => {
    ...
    const fakeDrawio = path.join(
      dir,
      process.platform === "win32" ? "drawio.exe" : "drawio",
    );
    fs.writeFileSync(
      input,
      `<mxfile><diagram><mxGraphModel><root><mxCell id="0"/><mxCell id="1" parent="0"/></root></mxGraphModel></diagram></mxfile>`,
    ...
    assert.deepEqual(report.command.slice(1), [
      "-x",
      "-f",
      "png",
      "-s",
...
test("drawio-export dry-run can discover drawio from PATH", () => {
  withTempDir((dir) => {
    ...
    assert.equal(report.ok, true);
    assert.equal(
      normalizeExecutablePath(report.command[0]),
      normalizeExecutablePath(path.resolve(fakeDrawio)),
    );
  });
});
...
test("drawio-export reports install guidance when CLI is missing", () => {
  withTempDir((dir) => {
    ...
    const result = spawnSync(
      process.execPath,
      [
        path.join(scriptsDir, "drawio-export.mjs"),
        input,
        "--drawio",
        missing,
        "--dry-run",
...
function readCreatePayload(url: string): {
  type: string;
  compressed: boolean;
  data: string;
} {
...
function inflateCreatePayload(url: string): string {
  ...
  return decodeURIComponent(
    zlib.inflateRawSync(Buffer.from(payload.data, "base64")).toString("utf8"),
  );
}
...
test("png-embed-fix appends missing IEND tail and is idempotent", () => {
  withTempDir((dir) => {
    ...
    const first = JSON.parse(run("png-embed-fix.mjs", [file, "--json"])) as {
      changed: boolean;
    };
    const second = JSON.parse(run("png-embed-fix.mjs", [file, "--json"])) as {
      changed: boolean;
    };
...
test("shape and brand tools return stable matches", () => {
  const shapes = JSON.parse(
    run("shape-query.mjs", ["aws lambda", "--limit", "3", "--format", "json"]),
  ) as Array<{ title: string }>;
  assert.ok(shapes.some((shape) => /lambda/i.test(shape.title)));

  const brands = JSON.parse(
    run("brand-symbols.mjs", ["claude", "--json"]),
  ) as Array<{ brand: string; style: string }>;
  assert.equal(brands[0].brand, "claude");
  assert.match(brands[0].style, /shape=image/);

  const unknown = spawnSync(
    process.execPath,
    [path.join(scriptsDir, "brand-symbols.mjs"), "zzzzzzzzzz", "--json"],
    {
      encoding: "utf8",
    },
  );
...
test("module scanners produce small dependency graphs", () => {
  withTempDir((dir) => {
    ...
    const js = JSON.parse(run("scan-js-modules.mjs", [dir])) as {
  ...
  withTempDir((dir) => {
    ...
    const ts = JSON.parse(run("scan-ts-modules.mjs", [dir])) as {
      edges: Array<{ source: string; target: string }>;
    };
  ...
  withTempDir((dir) => {
    ...
    const py = JSON.parse(run("scan-python-modules.mjs", [dir])) as {
      edges: Array<{ source: string; target: string }>;
    };
    const classes = JSON.parse(run("scan-python-classes.mjs", [dir])) as {
      edges: Array<{ source: string; target: string }>;
    };
  ...
  withTempDir((dir) => {
    ...
    fs.writeFileSync(
      path.join(dir, "a", "a.go"),
      'package a\nimport _ "example.com/m/b"\n',
    );
    fs.writeFileSync(path.join(dir, "b", "b.go"), "package b\n");
    const go = JSON.parse(run("scan-go-packages.mjs", [dir])) as {
      edges: Array<{ source: string; target: string }>;
    };
    assert.deepEqual(go.edges, [
  ...
  withTempDir((dir) => {
    ...
    const rust = JSON.parse(run("scan-rust-modules.mjs", [dir])) as {
      edges: Array<{ source: string; target: string }>;
    };
...
test("layout-graph reports missing Graphviz without affecting other tools", () => {
  withTempDir((dir) => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(scriptsDir, "layout-graph.mjs"), graph],
      {
