...
const interactiveScript = path.join(
  root,
  "skills",
  "fec-image-generation",
  "scripts",
  "interactive-diagram-server.mjs",
);
...
test("interactive-diagram-server starts, serves HTML, isolates sessions, and exports JSON", async () => {
  ...
  try {
    ...
    assert.equal(
      (
        await postCommand(server.port, sessionA, {
          cmd: "init",
          title: "A",
          direction: "TB",
        })
    ...
    assert.equal(
      (
        await postCommand(server.port, sessionB, {
          cmd: "init",
          title: "B",
          direction: "LR",
        })
    ...
    const stateA = JSON.parse(
      (await httpRequest(server.port, "GET", `/state?s=${sessionA}`)).body,
    ) as Array<{ cmd: string }>;
    const stateB = JSON.parse(
      (await httpRequest(server.port, "GET", `/state?s=${sessionB}`)).body,
    ) as Array<{ cmd: string }>;
    ...
    assert.deepEqual(exported.graph.nodes.map((node) => node.id).sort(), [
      "done",
      "start",
    ]);
    assert.deepEqual(
      exported.graph.edges.map((edge) => `${edge.from}->${edge.to}`),
      ["start->done"],
    );
...
test("png-qa reports parseable json for a normal PNG", () => {
  ...
  try {
    ...
    writePng(png, 120, 80, (x, y) =>
      x >= 32 && x <= 88 && y >= 24 && y <= 56 ? black : white,
    );

    const result = spawnSync(
      process.execPath,
      [script, "--png", png, "--format", "json"],
      {
        cwd: root,
        encoding: "utf8",
      },
...
test("png-qa detects blank images and edge clipping", () => {
  ...
  try {
    ...
    writePng(clipped, 80, 60, (x, y) =>
      x <= 10 && y >= 20 && y <= 40 ? black : white,
    );

    const blankResult = spawnSync(
      process.execPath,
      [script, "--png", blank, "--format", "json"],
      {
        cwd: root,
        encoding: "utf8",
      },
    ...
    const clippedResult = spawnSync(
      process.execPath,
      [script, "--png", clipped, "--format", "markdown"],
      {
        cwd: root,
        encoding: "utf8",
      },
    ...
    const blankReport = JSON.parse(blankResult.stdout) as {
...
test("png-qa detects manifest overlap, connector collisions, stacking, and label overflow", () => {
  ...
  try {
    ...
    writePng(png, 400, 240, (x, y) =>
      x >= 40 && x <= 360 && y >= 40 && y <= 200 ? black : white,
    );
    fs.writeFileSync(
      manifest,
      JSON.stringify(
        {
          ...
          boxes: [
            { id: "a", x: 40, y: 80, width: 90, height: 50, label: "API" },
            {
              ...
              width: 90,
              height: 50,
              label: "Extremely long label that cannot fit",
            },
    ...
    const result = spawnSync(
      process.execPath,
      [script, "--png", png, "--manifest", manifest, "--format", "json"],
      {
        cwd: root,
        encoding: "utf8",
      },
    ...
    assert.ok(
      report.nextActions.some((action) =>
        /node spacing|split dense groups/i.test(action),
      ),
    );
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        edges: [
          { from: "pr", to: "test", label: "trigger", variant: "emphasis" },
        ],
      }),
      "utf8",
    );
    ...
    const layout = JSON.parse(fs.readFileSync(manifest, "utf8")) as {
      canvas: { width: number };
      boxes: unknown[];
      connectors: unknown[];
    };
...
test("tech-diagram-render creates process workflow nodes, summaries, waypoints, and offline HTML", () => {
  ...
  try {
        ...
        summary: [
          {
            title: "Inputs",
            type: "active",
            items: ["Purchase request", "Policy threshold"],
          },
          {
            title: "Outcomes",
            type: "success",
            items: ["Payment issued", "Requester notified"],
          },
        ],
      }),
      "utf8",
    ...
    const layout = JSON.parse(fs.readFileSync(manifest, "utf8")) as {
      ...
      connectors: Array<{ id: string; points: Array<[number, number]> }>;
    };
    assert.deepEqual(layout.boxes.map((box) => box.id).sort(), [
      "classify",
      "pay",
      "reject",
      "review",
    ...
    const rejection = layout.connectors.find((connector) =>
      connector.id.startsWith("review-reject"),
    );
    assert.ok(rejection);
    assert.deepEqual(rejection.points.slice(1, -1), [
      [650, 396],
      [520, 396],
    ]);
  } finally {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        meta: {
          ...
          subtitle: "Browser-ready system topology",
        },
        groups: [
          {
            id: "cloud",
        ...
        summary: [
          {
            ...
            type: "backend",
            items: [
              "Gateway routes checkout traffic",
              "Service writes orders and emits events",
            ],
          },
          {
            title: "Trust",
            type: "security",
            items: [
              "Auth flow is separate from order data",
              "Private service zone is grouped",
            ],
          },
        ],
      }),
...
test("tech-diagram-render supports themed architecture diagrams with semantic nodes and flow legend", () => {
  const tmp = fs.mkdtempSync(
    path.join(os.tmpdir(), "fec-themed-architecture-"),
  );
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        meta: {
          title: "Agent Runtime",
          subtitle: "Semantic technical diagram",
        },
        groups: [
          {
            id: "runtime",
...
test("tech-diagram-render reports architecture validation errors", () => {
  ...
  try {
    ...
    for (const sample of cases) {
      ...
      if (sample.expectedStdout)
        assert.match(result.stdout, sample.expectedStdout);
      if (sample.expectedStderr)
        assert.match(result.stderr, sample.expectedStderr);
    }
  } finally {
    fs.rmSync(tmp, { recursive: true, force: true });
  }
...
test("export-diagram extracts inline SVG from HTML and uses default output path", () => {
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [exportScript, "--input", html, "--format", "svg"],
      {
        cwd: root,
        encoding: "utf8",
      },
...
test("export-diagram accepts SVG input and explicit output path", () => {
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [exportScript, "--input", input, "--format", "svg", "--output", output],
      {
        cwd: root,
        encoding: "utf8",
      },
...
test("export-diagram fails clearly when HTML has no inline SVG", () => {
  ...
  try {
    ...
    fs.writeFileSync(
      html,
      `<!doctype html><html><body>No diagram</body></html>`,
      "utf8",
    );
...
test("export-diagram covers PNG and JPG raster paths with browser fallback errors", () => {
  ...
  try {
    ...
    fs.writeFileSync(
      input,
      `<svg width="96" height="72" viewBox="0 0 96 72"><rect width="96" height="72" fill="#ffffff"/><circle cx="48" cy="36" r="18" fill="#3157d5"/></svg>`,
      "utf8",
    );
    ...
    for (const [format, output] of [
      ...
      if (result.status === 0) {
        assert.ok(
          ...
          `${format} export should create a non-empty file`,
        );
      } else {
        assert.match(
          result.stderr,
          /requires a local Chromium browser|Browser raster export failed|JPG\/JPEG export needs a local image converter/,
        );
...
async function startInteractiveServer(): Promise<{
  child: ChildProcess;
  port: number;
}> {
  ...
  const port = await new Promise<number>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      reject(
        new Error(
          `interactive server did not start. stdout=${stdout} stderr=${stderr}`,
        ),
      );
    }, 5000);
    ...
    child.on("exit", (code) => {
      if (code !== null && code !== 0) {
        ...
        reject(
          new Error(
            `interactive server exited with ${code}. stdout=${stdout} stderr=${stderr}`,
          ),
        );
      }
    });
...
function postCommand(
  ...
  return httpRequest(
    port,
    "POST",
    `/cmd?s=${encodeURIComponent(session)}`,
...
function writePng(
  ...
  fs.writeFileSync(
    filePath,
    Buffer.concat([Buffer.from("89504e470d0a1a0a", "hex"), ...chunks]),
  );
}
