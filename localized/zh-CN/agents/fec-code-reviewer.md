...
1. **收集上下文** — 先选择模式。仅改动审核检查 `git diff --staged`、`git diff` 及相关未跟踪文件；无改动时说明情况并停止，不自动审核最近提交。指定范围或全项目审核建立目标清单，不要求差异。
2. **理解范围** — 按所选模式识别目标文件、对应功能/缺陷及与路由、状态、API 层的关联。
...
- **仅改动审核跳过**：未改动代码中的问题，**除非是 CRITICAL 安全项**（如已合并代码中存在硬编码密钥且仍在线上路径）。
