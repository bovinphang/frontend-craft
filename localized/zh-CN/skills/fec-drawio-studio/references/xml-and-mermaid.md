...
- 普通形状标签包含 `whiteSpace=wrap;html=1`，让长文本稳定换行。
- 转义 XML 属性值：`&`、`<`、`>` 和 `"` 必须写成实体。
...
- 最终交接前运行 `diagram-lint.mjs`。当导出产物是最终交付件时使用 `--strict`，让重叠、literal line break、缺少 wrapping 样式、字号过小、网格偏移和页面越界等 warning 阻断交付。
