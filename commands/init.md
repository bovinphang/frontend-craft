...
4. 复制完成后，提醒用户：
   ...
   - 在 `CLAUDE.md` 底部的规则导入部分，删除项目不使用的规则（如纯 React 项目删除 `@./rules/vue.md`，不需要 i18n 的项目删除 `@./rules/i18n.md`）；若项目有 CI/CD 流水线可保留 `@./rules/ci-cd.md`，若涉及从现有项目重构可保留 `@./rules/refactoring.md`
