...
| ---------------------------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| 指定目录评审       | `fec-code-review`                | 「合并前请评审 `src/features/checkout/`：架构、类型、无障碍和测试；把 Markdown 报告写到 `reports/`。」                                 |
...
代码和安全审核 skill 采用三种模式。在 OpenClaw 中使用自然语言提出请求；以下斜杠命令示例适用于提供 `/fec-review` 的运行时：
...
指定文件或目录默认审核其全部代码；加上“本次改动”才仅审核差异。自然语言请求“审核代码”即使有改动也默认全项目；开始时说明模式和范围，修改后自动委托审核须显式传入本次改动范围。
