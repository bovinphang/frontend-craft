...
2. Create chart specifications
   ...
   - Read [diagram-patterns.md](references/diagram-patterns.md) when the user specifies ERD, UML, Sequence, Architecture, ML/DL or Flowchart.
...
3. Generate `.drawio` source
   - Draw.io XML can be manually generated for small drawings to keep node coordinates, sizes and connections clear.
   ...
   - Code structure diagram can be used first [scan-js-modules.mjs](scripts/scan-js-modules.mjs), [scan-ts-modules.mjs](scripts/scan-ts-modules.mjs), [scan-python-modules.mjs](script s/scan-python-modules.mjs), [scan-go-packages.mjs](scripts/scan-go-packages.mjs), [scan-rust-modules.mjs](scripts/scan-rust-modules.mjs) or [scan-python-classes.mjs](scripts/scan-python-classes.mjs) to generate graph JSON.
