...
import { copyDir, copyFile, ensureDir, readUtf8, retireManagedFile } from "../shared/fs.js";
import { asRecord, mergeHooks, readSettings, writeSettings } from "../shared/settings.js";
...
export function buildHooksJson(pluginRoot: string): string {
  ...
  return JSON.stringify(JSON.parse(raw), (_key, value: unknown) =>
    typeof value === "string" ? injectPluginRoot(pluginRoot, value) : value, 2);
}
...
export async function installClaude(ctx: InstallContext): Promise<void> {
  ...
  writeSettings(settingsPath, mergeHooks(readSettings(settingsPath), hooks), hooks);
