...
import { ensureDir, readUtf8, writeUtf8, retireManagedFile } from "../shared/fs.js";
...
export async function installCopilot(ctx: InstallContext): Promise<void> {
...

  {
    ...
    /** @type {string[]} */
    const parts = [isGlobal ? "# Frontend Craft - Copilot instructions\n" : '---\napplyTo: "**"\n---\n\n# Frontend Craft - Copilot instructions\n'];
    ...
    const dest = isGlobal ? path.join(baseDir, "copilot-instructions.md") : path.join(instr, "frontend-craft.instructions.md");
    if (!isGlobal || !fs.existsSync(dest)) writeUtf8(dest, parts.join(""));
  }

  if (isGlobal) {
    console.log("Copilot global: installed CLI instructions; IDE prompt files require a local install.");
    return;
  }
  ensureDir(prompts);
  ...
  for (const f of fs.readdirSync(cmdDir)) {
    ...
    writeUtf8(path.join(prompts, f.replace(/\.md$/, ".prompt.md")), readUtf8(path.join(cmdDir, f)));
